...
class STDBSCAN(object):
    ...
    def projection(self, df, p1_str='epsg:4326', p2_str='epsg:3857'):
        """
    ...
    def _retrieve_neighbors(self, index_center, matrix):
        ...
        # filter by time
        min_time = center_point[2] - timedelta(seconds=self.temporal_threshold).total_seconds()

        max_time = center_point[2] + timedelta(seconds=self.temporal_threshold).total_seconds()

        matrix = matrix[(matrix[:, 2] >= min_time) & (matrix[:, 2] <= max_time), :]
        ...
        return [int(i) for i in neigborhood]
